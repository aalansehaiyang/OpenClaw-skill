...
version: 1.1.0
description: 下载微信公众号文章并上传到飞书文档（含图片）。一键将微信文章内容（含标题、段落、列表、图片）导入到指定飞书文档。
---

# weixin_content_to_feishu — 微信公众号文章 → 飞书文档

**一键下载微信公众号文章并写入飞书文档**，自动处理标题、图片和位置还原。
...
1. **下载** — 调用 `web-content-download` 下载微信文章（含图片），保存为 Markdown
2. **写入** — 使用 `lark-cli docs +update --command overwrite` 将 Markdown 全文覆盖到飞书文档，标题后自动添加原文链接
3. **图片上传** — 自动识别图片占位符，上传到飞书并还原到原文位置
4. **修复标题** — 自动提取文章标题并替换默认 "Untitled"，清理占位符
