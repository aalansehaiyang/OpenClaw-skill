...
import { fetchPublishedArticles, formatArticlesForFooter } from "./fetch-latest-articles.js";
...
Options:
  ...
  --latest-articles   Append latest published articles to the end (requires API credentials)
  --latest-articles-count <n>  Number of latest articles to append (default: 6)
  --latest-articles-account <name>  Account name for footer (default: 破局哥聊职场)
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--type" && argv[i + 1]) {
      ...
      args.dryRun = true;
    } else if (arg === "--latest-articles") {
      args.latestArticles = true;
    } else if (arg === "--latest-articles-count" && argv[i + 1]) {
      args.latestArticlesCount = parseInt(argv[++i]!, 10);
    } else if (arg === "--latest-articles-account" && argv[i + 1]) {
      args.latestArticlesAccount = argv[++i]!;
...
async function main(): Promise<void> {
  ...
  // Append latest articles if requested
  if (args.latestArticles) {
    console.error(`[wechat-api] Fetching ${args.latestArticlesCount} latest articles from "${args.latestArticlesAccount}"...`);
    try {
      const articles = await fetchPublishedArticles(accessToken, 0, args.latestArticlesCount);
      const footerHtml = formatArticlesForFooter(articles, args.latestArticlesAccount);
      if (footerHtml) {
        htmlContent = htmlContent + footerHtml;
        console.error(`[wechat-api] Appended ${articles.length} latest articles to footer`);
      }
    } catch (err) {
      console.error(`[wechat-api] Warning: Failed to fetch latest articles: ${err instanceof Error ? err.message : String(err)}`);
      console.error(`[wechat-api] Continuing without article recommendations`);
    }
  }
